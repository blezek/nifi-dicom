  ...
  public ValidationResult validate(String subject, String value, ValidationContext context) {
    if (context.isExpressionLanguageSupported(subject) && context.isExpressionLanguagePresent(value)) {
      ...
      if (!resultType.equals(ResultType.STRING)) {
        return new ValidationResult.Builder().subject(subject).input(value).valid(false).explanation(
            "Expected Attribute Query to return type " + ResultType.STRING + " but query returns type " + resultType)
            .build();
      }
    }
    if (!value.equals("*") && (value.length() > 16 || !Pattern.matches("[a-zA-z0-9_-]*", value))) {
      return new ValidationResult.Builder().subject(subject).input(value)
          .explanation("AE Title must be blank or up to 16 charagters long, containing only [a-zA-Z0-9_-]").valid(false)
          .build();
    }

...
public class ExtractDICOMTags extends AbstractProcessor {
	...
	for (FlowFile flowFile : session.get(100)) {
		...
		try (DicomInputStream in = new DicomInputStream(flowfileInputStream)) {
			...
			builder.append(attributes.getString(Tag.PatientName, "unknown_patient")
				.replaceAll(badCharacters, "_"));
			builder.append("/");

			// Modality / Date
			builder.append(
				attributes.getString(Tag.StudyDate, "unknown_date").replaceAll(badCharacters, "_"));
			builder.append("_");
			builder.append(
				attributes.getString(Tag.Modality, "unknown_modality").replaceAll(badCharacters, "_"));
			builder.append("/");

			// Series
			builder.append(attributes.getString(Tag.SeriesNumber, "9999").replaceAll(badCharacters, "_"));
			builder.append("_");
			builder.append(attributes.getString(Tag.SeriesDescription, "no_series_description")
				.replaceAll(badCharacters, "_"));
			builder.append("/");
			...
			for (PropertyDescriptor descriptor : properties.keySet()) {
				...
				if (attributes.contains(tag) && attributes.containsValue(tag)) {
				    ...
				    attributeMap.put(tagName, context.getProperty(descriptor)
					    .evaluateAttributeExpressions(flowFile).toString());
				}
			    }
			}
		    }

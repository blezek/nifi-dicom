...
<xsl:template name="CheckContentItem">
	...
	<!-- matchlist is filtered by match to either $cvConceptName and $csdConceptName, or entries in $conceptNameCID, if present, or not filtered if neither are present (i.e., match any concept if no constraints are specified) -->
	<xsl:variable name="matchlist" select="child::node()[
		...
		and (
				(string-length($conceptNameCID) = 0 and string-length($cvConceptName) = 0)

...
/**
 * <p>An abstract class that provides the basis for posting the position of specified
 * slices and volumes on (usually orthogonal) localizer images.</p>
 *
 * <p>This base class provides the interface, common storage and various utility methods,
 * and specific methods of performing the localization operation are performed by concrete
 * sub-classes, instantiated through a factory class.</p>
 *
 * <p>Typically this would b used as follows, to get outlines in the form of a vector
 * of shapes whose coordinates are those of the localizer image:</p>
 * <pre>
...
public abstract class LocalizerPoster {
	...
	private static final String identString = "@(#) $Header: /userland/cvs/pixelmed/imgbook/com/pixelmed/geometry/LocalizerPoster.java,v 1.21 2017/01/24 10:50:43 dclunie Exp $";
	...
	/***/
	protected Tuple3d localizerVoxelSpacing;	// row spacing (between centers of adjacent rows), then column spacing, then slice spacing
	/***/
	...
	/**
	 * <p>Check that the row and column vectors are unit vectors and are orthogonal.</p>
	 *
	...
	/**
	 * <p>Check that the row and column and slice direction vectors are unit vectors and are orthogonal.</p>
	 *
	 * @param	row				the row direction cosines
	 * @param	column				the column direction cosines
	...
	/**
	 * <p>Get the corners of a slice in the 3D coordinate space of that slice.</p>
	 *
	 * @param	g	the geometry of a slice
	...
	public static Point3d[] getCornersOfSourceRectangleInSourceSpace(GeometryOfSlice g) {
		return getCornersOfSourceRectangleInSourceSpace(g.getRow(),g.getColumn(),g.getTLHC(),g.getVoxelSpacing(),g.getDimensions());
	}
	 ...
	 * @param	originalTLHC	the position of the top left hand corner of the slice as a point (X, Y and Z) LPH+
	...
	public static Point3d[] getCornersOfSourceRectangleInSourceSpace(
			Vector3d row,Vector3d column,Point3d originalTLHC,Tuple3d voxelSpacing,Tuple3d dimensions) {
...
//System.err.println("LocalizerPoster.getCornersOfSourceRectangleInSourceSpace():Column vector = "+column);
		...
		//Vector3d normal= new Vector3d(); normal.cross(row,column);	// the normal to the plane is the cross product of the row and column
//System.err.println("LocalizerPoster.getCornersOfSourceRectangleInSourceSpace():Normal vector = "+normal);

		double    spacingArray[] = new double[3];  voxelSpacing.get(spacingArray);
		double dimensionsArray[] = new double[3];    dimensions.get(dimensionsArray);

		Vector3d    distanceAlongRow = new Vector3d(row);       distanceAlongRow.scale((dimensionsArray[1]/*cols*/)*spacingArray[1]/*between cols*/);
		Vector3d distanceAlongColumn = new Vector3d(column); distanceAlongColumn.scale((dimensionsArray[0]/*rows*/)*spacingArray[0]/*between rows*/);
...
//System.err.println("LocalizerPoster.getCornersOfSourceRectangleInSourceSpace():Distance along a column = "+distanceAlongColumn);
		...
		Point3d trhc = new Point3d(tlhc); trhc.add(distanceAlongRow);
		Point3d blhc = new Point3d(tlhc); blhc.add(distanceAlongColumn);
		Point3d brhc = new Point3d(tlhc); brhc.add(distanceAlongRow); brhc.add(distanceAlongColumn);
...
//System.err.println("LocalizerPoster.getCornersOfSourceRectangleInSourceSpace():BRHC = "+brhc);
	...
	/**
	 * <p>Get the corners of a volume in the 3D coordinate space of that volume.</p>
	 *
	 * @param	row		the direction of the row as X, Y and Z components (direction cosines, unit vector) LPH+
	 * @param	column		the direction of the column as X, Y and Z components (direction cosines, unit vector) LPH+
	 * @param	originalTLHC	the position of the top left hand corner of the slice as a point (X, Y and Z) LPH+
	 ...
	 * @return			an array of eight points that are the tlhcT, trhcT, brhcT, blhcT, tlhcB, trhcB, brhcB, blhcB of the volume
	 */
	public static Point3d[] getCornersOfSourceCubeInSourceSpace(
			Vector3d row,Vector3d column,Point3d originalTLHC,Tuple3d voxelSpacing,double sliceThickness,Tuple3d dimensions) {
...
//System.err.println("LocalizerPoster.getCornersOfSourceCubeInSourceSpace(): Normal vector = "+normal);
		...
		Vector3d    distanceAlongRow = new Vector3d(row);       distanceAlongRow.scale((dimensionsArray[1]/*cols*/)*spacingArray[1]/*between cols*/);
		Vector3d distanceAlongColumn = new Vector3d(column); distanceAlongColumn.scale((dimensionsArray[0]/*rows*/)*spacingArray[0]/*between rows*/);
		Vector3d distanceAlongNormal = new Vector3d(normal); distanceAlongNormal.scale((dimensionsArray[2]/2)*sliceThickness);	// divide by two ... half on either side of center
...
//System.err.println("LocalizerPoster.getCornersOfSourceCubeInSourceSpace(): Distance along normal = "+distanceAlongNormal);
		...
		Point3d trhcT = new Point3d(tlhcT); trhcT.add(distanceAlongRow);
		Point3d blhcT = new Point3d(tlhcT); blhcT.add(distanceAlongColumn);
		Point3d brhcT = new Point3d(tlhcT); brhcT.add(distanceAlongRow); brhcT.add(distanceAlongColumn);
...
//System.err.println("LocalizerPoster.getCornersOfSourceCubeInSourceSpace(): Top    BRHC = "+brhcT);
		...
		Point3d trhcB = new Point3d(tlhcB); trhcB.add(distanceAlongRow);
		Point3d blhcB = new Point3d(tlhcB); blhcB.add(distanceAlongColumn);
		Point3d brhcB = new Point3d(tlhcB); brhcB.add(distanceAlongRow); brhcB.add(distanceAlongColumn);
...
//System.err.println("getIntersectionOfRectanglesInXYPlane(): BLHC intersection = "+newPoints[3]);
	...
	static public Rectangle getBoundsOfContainedRectangle(Point3d[] containedRectangle,Point3d[] wholeRectangle,Rectangle boundsOfWholeRectangle) {
		Rectangle boundsOfContainedRectangle = new Rectangle();

		double[] wholeTLHC = new double[3]; wholeRectangle[0].get(wholeTLHC);
		double[] wholeBRHC = new double[3]; wholeRectangle[2].get(wholeBRHC);
		double wholeHeight = wholeBRHC[1]-wholeTLHC[1];
		double wholeWidth  = wholeBRHC[0]-wholeTLHC[0];
		// assert wholeHeight > 0
		// assert wholeWidth > 0
		
		double[] containedTLHC = new double[3]; containedRectangle[0].get(containedTLHC);
		double[] containedBRHC = new double[3]; containedRectangle[2].get(containedBRHC);
		...
		boundsOfContainedRectangle.x = (int)((containedTLHC[0] - wholeTLHC[0])/wholeHeight * boundsOfWholeRectangle.width);
		boundsOfContainedRectangle.y = (int)((containedTLHC[1] - wholeTLHC[1])/wholeWidth  * boundsOfWholeRectangle.height);
...
//System.err.println("getBoundsOfContainedRectangle(): boundsOfContainedRectangle = "+boundsOfContainedRectangle);
	...
	/**
	 * <p>Transform a set of points into the "viewport" defined by an origin and row and column vectors
	 * in the same 3D space, by shifting the points to the origin (TLHC) and rotating.</p>
	 *
	 * @param	points		an array of 3D points to be transformed
...
//System.err.println("transformPointFromSourceSpaceIntoLocalizerSpace: ["+i+"] After rotation, point (mm) = "+newPoint);
	...
	/**
	 * <p>Transform a point into the "viewport" defined by an origin and row and column vectors
	 * in the same 3D space, by shifting the points to the origin (TLHC) and rotating.</p>
	 *
...
//System.err.println("transformPointFromSourceSpaceIntoLocalizerSpace: After rotation, point (mm) = "+newPoint);
	...
	/**
	 * <p>Transform a point into the "viewport" defined by the localizer that we are an instance of.</p>
	 *
	 * @param	point		a 3D point to be transformed
	 * @return			a new, transformed point
	 */
...
//System.err.println("transformPointFromSourceSpaceIntoLocalizerSpace: After rotation, point (mm) = "+newPoint);
	...
	/**
	 * <p>Project a point in localizer 3D space onto the plane of the localizer by ignoring
	 * the Z component, and return the X and Y coordinates as image-TLHC relative
	 * column and row offsets.</p>
	 *
	 * <p>Will return sub-pixel values ranging from 0.5 to 0.5 less than the maximum
	 * dimensions of the image, which allows points at the very edges of the rendered
	 * image to be drawn (e.g. a column of 0.5 will draw at the extreme left and a
	 * column of 255.5 will draw at the extreme right of a 256 pixel wide image (whereas
	 * 256.0 will not, though 0.0 will).</p>
	 *
...
//System.err.println("LocalizerPoster.transformPointInLocalizerPlaneIntoImageSpace: point   (row, column, in mm)       = ("+tmpArray3[1]+","+tmpArray3[0]+")");
		double scaleSubPixelHeightOfColumn=(localizerDimensionsArray[0]-1)/localizerDimensionsArray[0];	// number of rows
		double scaleSubPixelWidthOfRow    =(localizerDimensionsArray[1]-1)/localizerDimensionsArray[1];	// number of cols
		Point2D.Double location = new Point2D.Double(
			tmpArray3[0]/localizerVoxelSpacingArray[1]*scaleSubPixelHeightOfColumn+0.5,		// NB. x is the column; use as height number of rows * spacing between rows
			tmpArray3[1]/localizerVoxelSpacingArray[0]*scaleSubPixelWidthOfRow+0.5);		// NB. y is the row;    use as width  number of cols * spacing between cols
...
//System.err.println("U: ("+u[0]+","+u[1]+",0)");
	...
	protected Point3d intersectLineBetweenTwoPointsWithPlaneWhereZIsZero(Point3d aP,Point3d bP) {
		double[] a = new double[3]; aP.get(a);
		double[] b = new double[3]; bP.get(b);
...
//System.err.println("["+i+","+next+"]: ");
			corners[i].get(thisArray);    double thisZ = thisArray[2];
			corners[next].get(nextArray); double nextZ = nextArray[2];
	...
	protected static boolean classifyCornersIntoEdgeCrossingZPlane(Point3d startCorner,Point3d endCorner) {
		...
		startCorner.get(startArray); double startZ = startArray[2];
		    endCorner.get(endArray); double   endZ =   endArray[2];
...
//System.err.println("LocalizerPoster.getIntersectionsOfCubeWithZPlane(): no opposite edges intersect the Z plane");
	...
	protected void doCommonConstructorStuff() {
		...
		localizerNormal.cross(localizerRow,localizerColumn);	// the normal to the plane is the cross product of the row and column
	...
	/**
	 * <p>Established the geometry of the localizer image to be posted.</p>
	 *
	 * @param	row		the unit vector (direction cosine) of the row direction in the DICOM coordinate system
	 * @param	column		the unit vector (direction cosine) of the row direction in the DICOM coordinate system
	 * @param	tlhc		the position in the DICOM coordinate system of the center of the top left hand corner of the image
	 * @param	voxelSpacing	the row and column pixel spacing in mm
	 * @param	dimensions	the number of rows and columns
	 */
	public void setLocalizerGeometry(
	...
	/**
	 * <p>Get the shapes on the localizer of the specified slice.</p>
	 *
	 * @param	row		the unit vector (direction cosine) of the row direction in the DICOM coordinate system
	 * @param	column		the unit vector (direction cosine) of the row direction in the DICOM coordinate system
	 * @param	tlhc		the position in the DICOM coordinate system of the center of the top left hand corner pixel of the slice
	 ...
	 * @return			vector of shapes {@link java.awt.Shape java.awt.Shape} to be drawn in the localizer row and column coordinates
	 */
	public abstract Vector getOutlineOnLocalizerForThisGeometry(
			Vector3d row,Vector3d column,Point3d tlhc,Tuple3d voxelSpacing,double sliceThickness,Tuple3d dimensions);
	
	/**
	 * <p>Get the shape on the localizer of a zero-thickness slice specified by the geometry of a 2D rectangle.</p>
	 *
	 * @param	geometry
	 * @return			vector of shapes {@link java.awt.Shape java.awt.Shape} to be drawn in the localizer row and column coordinates
	 */
	...
	private static Vector3d[] getOrthogonalVectors(double normalX,double normalY,double normalZ) {
		...
		Vector3d column = new Vector3d(); column.cross(normal,row);
		//column.normalize();
...
//System.err.println("Normal = "+normal);
	...
	/**
	 * <p>Get the shape on the localizer of one or more volume localization slabs.</p>
	 *
	...
	public final Vector getOutlineOnLocalizerForThisVolumeLocalization(SpectroscopyVolumeLocalization spectroscopyVolumeLocalization) {
		...
		if (spectroscopyVolumeLocalization != null) {
			...
			for (int j=0; j<n; ++j) {
				...
				Vector3d    distanceAlongRow = new Vector3d(row);       distanceAlongRow.scale(50000);
				Vector3d distanceAlongColumn = new Vector3d(column); distanceAlongColumn.scale(50000);
				tlhc.sub(distanceAlongRow);
				tlhc.sub(distanceAlongColumn);

				Point3d[] corners = getCornersOfSourceCubeInSourceSpace(row,column,tlhc,voxelSpacing,slabThickness,dimensions);
				for (int i=0; i<8; ++i) {
